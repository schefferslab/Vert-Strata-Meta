...
dat <- read_csv("data/stripped_data/final/data_joined.csv") %>%
  ...
  dplyr::mutate(composite_weight = bin_weight*weight*indepedence_weight) %>%
...
model_formulas_ch <- list("scaled_met ~ 1 + (strata | link)",
                          ...
                          "scaled_met ~ poly(strata, 2) + method + continent + elevation + (strata | link)",
...
mods_rich <- lapply(model_formulas, function(x){
  glmmTMB(x, family = beta_family(link = "logit"), weights = composite_weight, data = rich)
})
...
mods_abund <- lapply(model_formulas, function(x){
  glmmTMB(x, family = beta_family(link = "logit"), weights = composite_weight, data = abund)
})
...
col_2_slope_rich_est_cont <- ggplot(taxa_link_slope_rich, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  scale_y_continuous(limits = c(-13, 12), breaks = seq(-12, 12, by = 2), expand = c(0.05, 0),
                     sec.axis = sec_axis(~., breaks = c(-8,0,8), labels = c("Decreasing"," ","Increasing"))) +
...
col_2_slope_abund_est_cont <- ggplot(taxa_link_slope_abund, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  scale_y_continuous(limits = c(-14.78,14.5), breaks = seq(-14, 14, by = 2), expand = c(0.05, 0),
                     sec.axis = sec_axis(~., breaks = c(-7,0,7), labels = c("Decreasing"," ","Increasing"))) +
  geom_text(aes(x = order, y = if_else(sign(estimate) == 1, CI_95_upper + 1, CI_95_lower - 1), label = sig), show.legend = F) +
  scale_color_viridis_d("Taxa") + theme_classic() + 
  theme(legend.position = "none", axis.text.x.top = element_text(size = 8),
        axis.ticks.length.x.top = unit(0, "cm"),
...
col_2_slope_rich_avg_cont <- ggplot(taxa_link_slope_rich_avg, 
  ...
  scale_y_continuous(limits = c(-14, 14), breaks = seq(-14, 14, by = 2), expand = c(0.05, 0), 
                     sec.axis = sec_axis(~., breaks = c(-7,0,7), labels = c("Decreasing"," ","Increasing"))) +
  scale_color_viridis_d("Taxa") + theme_classic() + 
...
col_2_slope_abund_avg_cont <- ggplot(taxa_link_slope_abund_avg, 
  ...
  scale_x_discrete(labels = rev(c("Bats", "Birds", "Amphibians", "      Small Mammals"))) +
  scale_y_continuous(limits = c(-14.78,14.5), breaks = seq(-14, 14, by = 2), expand = c(0.05, 0), 
                     sec.axis = sec_axis(~., breaks = c(-7,0,7), labels = c("Decreasing"," ","Increasing"))) +  scale_color_viridis_d(" ") + theme_classic() + 

...
col_2_slope_abund_avg_cont <- ggplot(taxa_link_slope_abund_avg, 
                                aes(x = factor(taxa_order, levels = c(
                                "Small mammals", "Amphibians", "Birds", "Bats")), y = estimate, col = taxa_order)) + 
  ylab(expression(Delta ~ "metric with 1% increase in height")) + xlab(" ") + coord_flip() + 
  geom_hline(yintercept = 0, alpha = 0.5, size = 0.8) + geom_point(size = 1.5) + 
  geom_linerange(aes(ymax = CI_80_upper, 
                     ymin = CI_80_lower), size = 1.1) +
...
sum_mods_cat_list <- list(
  ...
  lm(estimate ~ continent,        data = bat_abund_slope), #16 *
...
emmeans::emmeans(sum_mods_cat_list[[16]], pairwise~continent)$contrasts
 # contrast                    estimate    SE df t.ratio p.value
 # Africa - Americas             0.9998 0.839 19  1.192  0.4719 
 # Africa - Asia and Oceania    -0.0419 0.891 19 -0.047  0.9988 
 # Americas - Asia and Oceania  -1.0417 0.417 19 -2.499  0.0543          # Bat abundance in the Americas is less upwardly stratified than Asia and Oceania 
...
emmeans::emmeans(sum_mods_cat_list[[21]], pairwise~forest_type_iucn)$contrasts
 ...
 # dry_forest - moist_montane_forest              0.877 0.331 21 2.649   0.0383 
 # moist_lowland_forest - moist_montane_forest    0.769 0.311 21 2.474   0.0550 # Bird abundance in the moist lowland forest is more upwardly stratified than moist montane forest

...
bbmle::AICtab(mods_rich, mnames = unlist(rich_mod_names))
```
...
### Getting model estimates from each model
```{r}
...
### Model averaging model estimates 
```{r}
...
## staggered richness and abundance plots (I don't like this...)
```{r}
...
### More intercept and slope figures (I don't like this so much either...)
```{r}
# ## Two panel figure
# pos_dodge <- 0.8
# 
...
taxa_link_slope_abund_avg <- taxa_link_slope_avg %>%
  ...
  rowid_to_column("order")
```
### Creating 4 panel plots of model estimates
```{r}
col_2_slope_rich_avg <- ggplot(taxa_link_slope_rich_avg, aes(x = rev(reorder(taxa_order, estimate)), y = estimate, col = taxa_order)) + 
  ...
  geom_linerange(aes(ymin = estimate - 0.1,
                     ymax = estimate + 0.1), size = 2) +
  geom_text(aes(x = rev(taxa_order), y = if_else(sign(estimate) == 1, CI_95_upper + 1, CI_95_lower - 1), label = sig), show.legend = F) +
...
col_2_slope_abund_avg <- ggplot(taxa_link_slope_abund_avg, aes(x = rev(reorder(taxa_order, estimate)), y = estimate, col = taxa_order)) + 
  ...
  theme(legend.position = "right", 
        axis.text.x.top = element_text(size = 2),
...
ggsave("analysis/figures/predictions_bio_parameters_link_taxa_4_panel_dodge.jpeg", width = 12, height = 10, units = "in", dpi = 300)
```

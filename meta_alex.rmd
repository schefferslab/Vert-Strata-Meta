...
## Loading data
```{r loading data}
read_csv("data/stripped_data/final/data_joined.csv") %>%
  glimpse()
...
dat <- read_csv("data/stripped_data/final/data_joined.csv") %>%
  ...
  dplyr::select(link, study_id.x, method, taxa, bin_weight, weight, indepedence_weight, 
                continent, biodiversity_metric, taxa_order, treatment, season, forest_type_iucn, 
...
model_formulas_ch <- list("scaled_met ~ 1 +                                                            weight + (strata | link)",
                          ...
                          "scaled_met ~ strata +             taxa + elevation +        canopy_height + weight + (strata | link)",
                          ...
                          "scaled_met ~ strata +             taxa + latitude +         elevation +     weight + (strata | link)",
...
ggsave("analysis/figures/model results/coef_richness_height.jpeg")
...
ggplot(taxa_link_rich_slope, aes(x = method, y = estimate, fill = taxa_order)) + 
  ...
  scale_color_viridis_d("Taxa") + theme_bw() + 
  theme(legend.position = "none")
ggsave("analysis/figures/model results/coef_richness_height_method.jpeg")
```
# Modeling abundance
## Abundance GLMs
...
ggplot(abund, aes(y = predictions_fit, x = canopy_height)) + geom_point() +
  ...
  facet_grid(rows = vars(taxa), cols = vars(continent)) + theme_bw() + 
  theme(legend.position = "bottom");
ggsave("analysis/figures/model results/abundance_strata_canopy_continent.jpeg", width = 8, height = 6, units = "in", dpi = 300)
...
ggplot(abund, aes(y = predictions_fit, x = latitude)) + geom_point() +
  ...
  facet_grid(rows = vars(taxa), cols = vars(continent)) + theme_bw() + 
  theme(legend.position = "bottom");
ggsave("analysis/figures/model results/coef_abund_height_latitude.jpeg", width = 8, height = 6, units = "in", dpi = 300)
...
ggplot(taxa_link_abund_slope, aes(x = mean_elevation, y = estimate, col = taxa_order)) + 
  ...
  scale_color_viridis_d("Taxa") + theme_bw() + 
  theme(legend.position = "none")
ggsave("analysis/figures/model results/coef_abund_height.jpeg")
...
ggplot(taxa_link_abund_slope, aes(x = method, y = estimate, fill = taxa_order)) + 
  ...
  scale_color_viridis_d("Taxa") + theme_bw() + 
  theme(legend.position = "none")
ggsave("analysis/figures/model results/coef_abund_height_method.jpeg")
```
...
col_2_slope_rich_est_cont <- ggplot(taxa_link_slope_rich, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  annotate("text", x = seq(56, 54), y = rep(2,3), 
...
ggsave("analysis/figures/model results/parm_taxa_4_panel.jpeg", width = 10, height = 10, units = "in", dpi = 300)
```
...
ggplot(taxa_link_biomet_sum, aes(y = log((pos_sig+1)/(neg_sig+1)), x = taxa_order, fill = taxa_order)) + 
  geom_hline(yintercept = 0, alpha = 0.5, size = 0.8) + ylim(c(-3,3)) +
...
ggsave("analysis/figures/model results/summary_slopes_taxa_biometric.jpeg", width = 8, height = 4, units = "in", dpi = 300)
```

## Ed--this is the section:
```{r}
# select taxa specific data

sm_rich_slope = subset(taxa_link_rich_slope, taxa_order == "Small mammals")
bat_rich_slope = subset(taxa_link_rich_slope, taxa_order == "Bats")
bird_rich_slope = subset(taxa_link_rich_slope, taxa_order == "Birds")
amph_rich_slope = subset(taxa_link_rich_slope, taxa_order == "Amphibians")

sm_abund_slope = subset(taxa_link_abund_slope, taxa_order == "Small mammals")
bat_abund_slope = subset(taxa_link_abund_slope, taxa_order == "Bats")
bird_abund_slope = subset(taxa_link_abund_slope, taxa_order == "Birds")
amph_abund_slope = subset(taxa_link_abund_slope, taxa_order == "Amphibians")

# creating linear models for  abundance slope estimate against the covariate
sm_abund_elev = summary(lm(sm_abund_slope$estimate ~ sm_abund_slope$mean_elevation))           ; m1 = cbind(sm_abund_elev$coefficients[2], sm_abund_elev$coefficients[8])
sm_abund_can = summary(lm(sm_abund_slope$estimate ~ sm_abund_slope$canopy_height))             ; m2 = cbind(sm_abund_can$coefficients[2], sm_abund_can$coefficients[8])
sm_abund_lat = summary(lm(sm_abund_slope$estimate ~ abs(sm_abund_slope$mean_latitude)))        ; m3 = cbind(sm_abund_lat$coefficients[2], sm_abund_lat$coefficients[8])

bat_abund_elev = summary(lm(bat_abund_slope$estimate ~ bat_abund_slope$mean_elevation))        ; m4 = cbind(bat_abund_elev$coefficients[2], bat_abund_elev$coefficients[8])
bat_abund_can = summary(lm(bat_abund_slope$estimate ~ bat_abund_slope$canopy_height))          ; m5 = cbind(bat_abund_can$coefficients[2], bat_abund_can$coefficients[8])
bat_abund_lat = summary(lm(bat_abund_slope$estimate ~ abs(bat_abund_slope$mean_latitude)))     ; m6 = cbind(bat_abund_lat$coefficients[2], bat_abund_lat$coefficients[8])

bird_abund_elev = summary(lm(bird_abund_slope$estimate ~ bird_abund_slope$mean_elevation))     ; m7 = cbind(bird_abund_elev$coefficients[2], bird_abund_elev$coefficients[8])
bird_abund_can = summary(lm(bird_abund_slope$estimate ~ bird_abund_slope$canopy_height))       ; m8 = cbind(bird_abund_can$coefficients[2], bird_abund_can$coefficients[8])
bird_abund_lat = summary(lm(bird_abund_slope$estimate ~ abs(bird_abund_slope$mean_latitude)))  ; m9 = cbind(bird_abund_lat$coefficients[2], bird_abund_lat$coefficients[8])

amph_abund_elev = summary(lm(amph_abund_slope$estimate ~ amph_abund_slope$mean_elevation))     ; m10 = cbind(amph_abund_elev$coefficients[2], amph_abund_elev$coefficients[8])
amph_abund_can = summary(lm(amph_abund_slope$estimate ~ amph_abund_slope$canopy_height))       ; m11 = cbind(amph_abund_can$coefficients[2], amph_abund_can$coefficients[8])
amph_abund_lat = summary(lm(amph_abund_slope$estimate ~ abs(amph_abund_slope$mean_latitude)))  ; m12 = cbind(amph_abund_lat$coefficients[2], amph_abund_lat$coefficients[8])

n1 = cbind(m1,m2,m3) ; n2 = cbind(m4,m5,m6) ; n3 = cbind(m7,m8,m9) ; n4 = cbind(m10,m11,m12)
o1 = round(as.data.frame(rbind(n1,n2,n3,n4)), 3) 
rownames(o1) <- c("Small mammals", "Bats","Birds" ,"Amphibians")
colnames(o1) <- c("Elev (slope)", "Elev (P)","Canopy (slope)", "Canopy (P)","Latitude (slope)", "Latitude (P)")
abund_stats = o1

# creating linear models for  richness slope estimate against the covariate

sm_rich_elev = summary(lm(sm_rich_slope$estimate ~ sm_rich_slope$mean_elevation))           ; p1 = cbind(sm_rich_elev$coefficients[2], sm_rich_elev$coefficients[8])
sm_rich_can = summary(lm(sm_rich_slope$estimate ~ sm_rich_slope$canopy_height))             ; p2 = cbind(sm_rich_can$coefficients[2], sm_rich_can$coefficients[8])
sm_rich_lat = summary(lm(sm_rich_slope$estimate ~ abs(sm_rich_slope$mean_latitude)))        ; p3 = cbind(sm_rich_lat$coefficients[2], sm_rich_lat$coefficients[8])

bat_rich_elev = summary(lm(bat_rich_slope$estimate ~ bat_rich_slope$mean_elevation))        ; p4 = cbind(bat_rich_elev$coefficients[2], bat_rich_elev$coefficients[8])
bat_rich_can = summary(lm(bat_rich_slope$estimate ~ bat_rich_slope$canopy_height))          ; p5 = cbind(bat_rich_can$coefficients[2], bat_rich_can$coefficients[8])
bat_rich_lat = summary(lm(bat_rich_slope$estimate ~ abs(bat_rich_slope$mean_latitude)))     ; p6 = cbind(bat_rich_lat$coefficients[2], bat_rich_lat$coefficients[8])

bird_rich_elev = summary(lm(bird_rich_slope$estimate ~ bird_rich_slope$mean_elevation))     ; p7 = cbind(bird_rich_elev$coefficients[2], bird_rich_elev$coefficients[8])
bird_rich_can = summary(lm(bird_rich_slope$estimate ~ bird_rich_slope$canopy_height))       ; p8 = cbind(bird_rich_can$coefficients[2], bird_rich_can$coefficients[8])
bird_rich_lat = summary(lm(bird_rich_slope$estimate ~ abs(bird_rich_slope$mean_latitude)))  ; p9 = cbind(bird_rich_lat$coefficients[2], bird_rich_lat$coefficients[8])

amph_rich_elev = summary(lm(amph_rich_slope$estimate ~ amph_rich_slope$mean_elevation))     ; p10 = cbind(amph_rich_elev$coefficients[2], amph_rich_elev$coefficients[8])
amph_rich_can = summary(lm(amph_rich_slope$estimate ~ amph_rich_slope$canopy_height))       ; p11 = cbind(amph_rich_can$coefficients[2], amph_rich_can$coefficients[8])
amph_rich_lat = summary(lm(amph_rich_slope$estimate ~ abs(amph_rich_slope$mean_latitude)))  ; p12 = cbind(amph_rich_lat$coefficients[2], amph_rich_lat$coefficients[8])

q1 = cbind(p1,p2,p3) ; q2 = cbind(p4,p5,p6) ; q3 = cbind(p7,p8,p9) ; q4 = cbind(p10,p11,p12)
r1 = round(as.data.frame(rbind(q1,q2,q3,q4)), 3) 
rownames(r1) <- c("Small mammals", "Bats","Birds" ,"Amphibians")
colnames(r1) <- c("Elev (slope)", "Elev (P)","Canopy (slope)", "Canopy (P)","Latitude (slope)", "Latitude (P)")
rich_stats = r1

all_stats = rbind(abund_stats, rich_stats)
all_stats$Metric = c(rep("Abundance", 4), rep("Richness", 4))

###
     ###   Now try to select only studies that did both abundance and richness and plot the estimates against one another
###

rich_studies  =  as.data.frame(unique(taxa_link_rich_slope$link)) ; colnames(rich_studies) = "link"
abund_studies  =  as.data.frame(unique(taxa_link_abund_slope$link)) ; colnames(abund_studies) = "link"
all_studies =  rbind(rich_studies, abund_studies)
all_studies_count = all_studies %>%
  count(link)
studies_matched = subset(all_studies_count, n == 2)

rich_studies_matched = taxa_link_rich_slope %>% 
  filter(link %in% studies_matched$link) %>% 
  mutate(estimate_rich = estimate) %>% 
  select(taxa_order, link, estimate_rich)

abund_studies_matched = taxa_link_abund_slope %>% 
  filter(link %in% studies_matched$link) %>% 
  mutate(estimate_abund = estimate) %>% 
  select(taxa_order, link, estimate_abund)

estimates_matched = merge(rich_studies_matched, abund_studies_matched, by = "link")

ggplot(estimates_matched, aes(x = estimate_abund, y = estimate_rich, color = taxa_order.x))+
         geom_point()+
...
ggsave("analysis/figures/study descriptions/abundance_richness_correlation.jpeg", width = 7, height = 5, units = "in", dpi = 300)
...
# summary_rich_coef[[4]] <- lmer(estimate ~ forest_type_iucn +   (forest_type_iucn|taxa_order), data = taxa_link_rich_slope)
# summary_rich_coef[[5]] <- lmer(estimate ~ season +             (season|taxa_order), data = taxa_link_rich_slope)
# 
...
# summary_abund_coef[[4]] <- lmer(estimate ~ forest_type_iucn +   (forest_type_iucn|taxa_order), data = taxa_link_abund_slope)
# summary_abund_coef[[5]] <- lmer(estimate ~ season +             (season|taxa_order), data = taxa_link_abund_slope)
# 

...
rich_beta_intercept <- matrix(NA, nrow = terms_length/2, ncol = length(mods_rich))
...
rich_term_length <- nrow(sjPlot::get_model_data(mods_abund[[1]], type = "re", se = T, transform = NULL))

abund_beta_intercept <- matrix(NA, nrow = rich_term_length/2, ncol = length(mods_abund))
...
for(i in 1:length(mods_abund)){
  abund_beta_intercept[,i] <- sjPlot::get_model_data(mods_abund[[i]], type = "re", se = T, transform = NULL)[1:(rich_term_length/2),] %>%
    dplyr::select(estimate) %>%
    as.matrix()
  
  abund_beta_intercept_se[,i] <- sjPlot::get_model_data(mods_abund[[i]], type = "re", se = T, transform = NULL)[1:(rich_term_length/2),] %>%
    dplyr::select(estimate, conf.low) %>%
    dplyr::mutate(se = (estimate - conf.low)/1.96) %>%
    dplyr::select(se) %>% 
    as.matrix()

  abund_beta_strata[,i] <- sjPlot::get_model_data(mods_abund[[i]], type = "re", se = T, transform = NULL)[((rich_term_length/2) + 1):rich_term_length,] %>%
    dplyr::select(estimate) %>%
    as.matrix()

  abund_beta_strata_se[,i] <- sjPlot::get_model_data(mods_abund[[i]], type = "re", se = T, transform = NULL)[((rich_term_length/2) + 1):rich_term_length,] %>%
    dplyr::select(estimate, conf.low) %>%
    dplyr::mutate(se = (estimate - conf.low)/1.96) %>%
    dplyr::select(se) %>% 
    as.matrix()
...
col_2_slope_rich_est_cont <- ggplot(taxa_link_slope_rich, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  scale_x_continuous(breaks = taxa_link_slope_rich$order, 
                     labels = taxa_link_slope_rich$link, expand = c(0.02, 0)) +
  scale_y_continuous(limits = c(-8,8), breaks = seq(-8, 8, by = 2), expand = c(0.05, 0),
...
col_2_slope_abund_est_cont <- ggplot(taxa_link_slope_abund, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  scale_x_continuous(breaks = taxa_link_slope_abund$order, 
                     labels = taxa_link_slope_abund$link, expand = c(0.02, 0)) +
...
```{r Creating taxa averaged slope estimate figures for both richness and abundance}
...
ggarrange(col_2_slope_est_cont, col_2_slope_avg_cont, nrow = 2, heights = c(6,1.5), hjust = -10)

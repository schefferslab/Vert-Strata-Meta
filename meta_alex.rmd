...
packages <-  c("lme4","AICcmodavg","glmmTMB","TMB","tidyverse","readr","scales",
               "formula.tools","sjPlot","ggpubr","Hmisc","visreg","bbmle")
...
dat <- read_csv("data/stripped_data/final/data_joined.csv") %>%
  ...
  dplyr::select(link, study_id.x, method, taxa, bin_weight, weight, indepedence_weight, 
                ...
                elevation, canopy_height, latitude, longitude, scaled_met, strata = mean_strata_height_p) %>%
  dplyr::mutate(composite_weight = as.numeric(bin_weight*weight*indepedence_weight)) %>%
  dplyr::select(-c(bin_weight, weight, indepedence_weight))

mean(scale(dat$composite_weight) + 1)
test <- scale(dat$composite_weight) + 1
test1 <- rescale(dat$composite_weight, to = c(1,2))
...
model_formulas_ch <- list("scaled_met ~ 1 + (strata | link)",
                          "scaled_met ~ strata + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + (strata | link)",

                          "scaled_met ~ strata + taxa + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + taxa + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + taxa + (strata | link)",

                          "scaled_met ~ strata + continent + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + continent + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + continent + (strata | link)",

                          "scaled_met ~ strata +          elevation + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + elevation + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + elevation + (strata | link)",

                          "scaled_met ~ strata +          longitude + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + longitude + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + longitude + (strata | link)",

                          "scaled_met ~ strata +          canopy_height + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + canopy_height + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + canopy_height + (strata | link)",

                          "scaled_met ~ strata +          latitude + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + latitude + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + latitude + (strata | link)",

                          "scaled_met ~ strata +          method + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + method + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + method + (strata | link)",

                          "scaled_met ~ strata +          elevation + continent + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + elevation + continent + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + elevation + continent + (strata | link)",

                          "scaled_met ~ strata +          elevation + latitude + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + elevation + latitude + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + elevation + latitude + (strata | link)",

                          "scaled_met ~ strata +          elevation + canopy_height + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + elevation + canopy_height + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + elevation + canopy_height + (strata | link)",

                          "scaled_met ~ strata +          latitude + canopy_height + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + latitude + canopy_height + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + latitude + canopy_height + (strata | link)",

                          "scaled_met ~ strata +          method + latitude + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + method + latitude + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + method + latitude + (strata | link)",

                          "scaled_met ~ strata +          method + canopy_height + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + method + canopy_height + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + method + canopy_height + (strata | link)",

                          "scaled_met ~ strata +          method + continent + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + method + continent + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + method + continent + (strata | link)",

                          "scaled_met ~ strata +          method + elevation + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + method + elevation + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + method + elevation + (strata | link)",
                          ...
                          "scaled_met ~ strata +          method + continent + elevation + (strata | link)",
                          "scaled_met ~ poly(strata, 2) + method + elevation + elevation + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + method + continent + elevation + (strata | link)",

                          "scaled_met ~ strata +          method + canopy_height + elevation + (strata | link)",
                          "scaled_met ~ poly(strata, 3) + method + continent + elevation + (strata | link)",
                          ...
                          "scaled_met ~ strata * method + continent + (strata | link)",
                          "scaled_met ~ strata * continent + method + (strata | link)",
                          "scaled_met ~ strata * continent + elevation + (strata | link)",
                          "scaled_met ~ strata * elevation + method + (strata | link)",
                          "scaled_met ~ strata * elevation + continent + (strata | link)")
...
(taxa_link_abund <- abund %>% 
  dplyr::group_by(taxa_order,link,continent,method) %>%
  dplyr::summarise(mean_elevation = mean(elevation),
                   mean_canopy_height = mean(canopy_height),
                   mean_latitude = mean(latitude)) %>%
...
ggplot(taxa_link_abund_order %>% filter(facet == "Change in abundance with increasing strata"), 
       aes(y = estimate, x = mean_canopy_height, col = taxa_order)) + 
  geom_point() + geom_smooth(method = "glm") +
...
col_2_abund_est <- ggplot(taxa_link_abund_order, aes(x = rev(order), y = estimate, col = taxa_order)) + 
  ...
  geom_linerange(aes(ymax = CI_95_upper,
                     ymin = CI_95_lower)) +
  scale_x_continuous(breaks = rev(taxa_link_abund_order$order[1:abund_length]), labels = taxa_link_abund_order$link[1:abund_length]) +
...
col_2_slope_abund_est_cont <- ggplot(taxa_link_slope_abund, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  scale_y_continuous(limits = c(-11.5, 11), breaks = seq(-10, 10, by = 2), expand = c(0.05, 0),
...
col_2_slope_rich_avg_cont <- ggplot(taxa_link_slope_rich_avg, 
  ...
  scale_y_continuous(limits = c(-8.5, 8.5), breaks = seq(-8, 8, by = 2), expand = c(0.05, 0), 
                     sec.axis = sec_axis(~., breaks = c(-4,0,4), labels = c("Decreasing"," ","Increasing"))) +
  scale_color_viridis_d("Taxa") + theme_classic() + 
...
col_2_slope_abund_avg_cont <- ggplot(taxa_link_slope_abund_avg, 
  ...
  scale_x_discrete(labels = rev(c("Bats", "Birds", "Amphibians", "      Small Mammals"))) +
  scale_y_continuous(limits = c(-11.5, 11), breaks = seq(-10, 10, by = 2), expand = c(0.05, 0), 
                     sec.axis = sec_axis(~., breaks = c(-5,0,5), labels = c("Decreasing"," ","Increasing"))) +  scale_color_viridis_d(" ") + theme_classic() + 
...
taxa_link_biomet_sum <- taxa_link_biomet_order %>%
  ...
  group_by(taxa_order) %>%
  dplyr::summarize(pos = sum(sign == 1),
                   neg = sum(sign == -1),
                   pos_sig = sum(sign == 1 & sig == "*"),
                   neg_sig = sum(sign == -1 & sig == "*"))



taxa_link_biomet_order
```

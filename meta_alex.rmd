...
var_bio_met <- read_csv("data/stripped_data/final/data_joined.csv") %>% 
  dplyr::group_by(link, biodiversity_metric) %>%
  dplyr::summarize(var_biodiversity_metric_mean = mean(var_biodiversity_metric))

sum(is.na(var_bio_met$var_biodiversity_metric_mean[var_bio_met$biodiversity_metric == "abundance"]))/nrow(var_bio_met)
sum(is.na(var_bio_met$var_biodiversity_metric_mean[var_bio_met$biodiversity_metric == "richness"]))/nrow(var_bio_met)
...
dat <- read_csv("data/stripped_data/final/data_joined.csv") %>%
  ...
  dplyr::mutate(weight = as.numeric(scale(weight)*indepedence_weight),
                forest_type_iucn = recode(forest_type_iucn, "dry forest" = "dry_forest"))
...
ggsave("analysis/figures/model results/parm_taxa_4_panel.jpeg", width = 10, height = 12, units = "in", dpi = 300)
```
...
ggplot(taxa_link_biomet_sum, aes(y = log((pos_sig+1)/(neg_sig+1)), x = factor(taxa_order, levels = c("Bats", "Birds", "Small mammals", "Amphibians")), 
                                 fill = factor(taxa_order, levels = c("Bats", "Birds", "Small mammals", "Amphibians")))) + 
  geom_hline(yintercept = 0, alpha = 0.5, size = 0.8) + ylim(c(-2,3)) +
  ...
  facet_wrap(~factor(facet, levels = c("Change in richness with increasing strata",
                                       "Change in abundance with increasing strata"))) + 
  scale_x_discrete(labels = c("Bats", "Birds", "Small mammals", "Amphibians")) +
  scale_fill_viridis_d("Taxa") + theme_classic() + theme(legend.position = "none") 
...
mod_sum_rich_pan <- ggplot(mod_sum_rich, aes(x = estimate, y = rev(order), color = taxa)) + 
  ...
  scale_x_continuous(limits = c(-6, 6.5), breaks = seq(-6, 6, by = 2),
...
ggsave("analysis/figures/model results/summary_slopes_vars.jpeg", width = 8, height = 4.5, units = "in", dpi = 300)
...
sum_mods_cat_list <- list(
  ...
  lm(estimate ~ continent,        data = bird_abund_slope),
  lm(estimate ~ season,           data = bird_abund_slope),
  ...
  lm(estimate ~ forest_type_iucn, data = amph_abund_slope)) #24 *

confint(sum_mods_cat_list[[1]])
...
which(mod_cat_sum$p_value < 0.05)

emmeans::emmeans(sum_mods_cat_list[[7]], pairwise~continent)$contrasts
 # contrast                    estimate    SE df t.ratio p.value
 # Africa - Americas             -0.166 0.645 26 -0.257  0.9643 
 # Africa - Asia and Oceania      0.911 0.661 26  1.378  0.3667 
 # Americas - Asia and Oceania    1.077 0.337 26  3.196  0.0098      # Bird richness in the Americas is more upwardly stratified than Asia and Oceania 

emmeans::emmeans(sum_mods_cat_list[[8]],  pairwise~season)$contrasts
 # contrast                estimate    SE df t.ratio p.value
 # Dry - Sampling combined    1.099 0.375 26  2.929  0.0185          # sampling combined in less upwardly stratified than data from the dry season 
 # Dry - Wet                 -0.364 0.418 26 -0.871  0.6633 
 # Sampling combined - Wet   -1.463 0.317 26 -4.611  0.0003          # sampling combined in less upwardly stratified than data from the wet season 

emmeans::emmeans(sum_mods_cat_list[[9]],  pairwise~forest_type_iucn)$contrasts
 # contrast                                    estimate    SE df t.ratio p.value
 # dry_forest - moist_lowland_forest             -0.532 0.412 26 -1.292  0.4123 
 # dry_forest - moist_montane_forest              0.717 0.387 26  1.853  0.1727 
 # moist_lowland_forest - moist_montane_forest    1.250 0.374 26  3.341  0.0069  # Bird richness in the moist lowland forest is more upwardly stratified than moist montane forest

emmeans::emmeans(sum_mods_cat_list[[21]], pairwise~forest_type_iucn)$contrasts
 # dry_forest - moist_lowland_forest              0.108 0.322 21 0.335   0.9403 
 # dry_forest - moist_montane_forest              0.877 0.331 21 2.649   0.0383 # Bird abundance in the moist lowland forest is more upwardly stratified than moist montane forest
 # moist_lowland_forest - moist_montane_forest    0.769 0.311 21 2.474   0.0550 # Bird abundance in dry forest is more upwardly stratified than moist montane forest

emmeans::emmeans(sum_mods_cat_list[[24]], pairwise~forest_type_iucn)$contrasts
# moist_lowland_forest - moist_montane_forest    0.643 0.145  4 4.423   0.0115 # Amphibian abundance in the moist lowland forest is more upwardly stratified than moist montane forest
...
ggsave("analysis/figures/study descriptions/abundance_richness_correlation.jpeg", width = 6, height = 4, units = "in", dpi = 300)
...
full_table_S1 =  merge(tables_S1, metrics_studies, by = "link")
...
###   making summary statistics for the 
```{r}
...
#here pulling in a different set of data columns from data_joined - including the references and things like that 
summary_dat <- read_csv("data/stripped_data/final/data_joined.csv") %>%
  dplyr::filter(taxa != "All mammals") %>%
  dplyr::filter(taxa != "Primates") %>%
  dplyr::mutate(weight = spatial_rank + temporal_bredth_rank + temporal_resolution_rank,
                taxa = as.factor(taxa),

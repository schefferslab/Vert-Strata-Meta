...
source("scripts/01_curation/00_package_loading.R")
...
bbmle::AICtab(mods_rich, mnames = unlist(rich_mod_names)) # Creating AIC table
```
...
```{r}
# Creating matrices for model predictions; 4 matrices, each with 50 rows (for each study) and 34 cols (for each model)
## 4 matrices:
### 1) Intercept
...
taxa_link_rich <- rich %>% 
  ...
  mutate(CI_99_lower = estimate - se*2.58,
...
# strata and elevation
mods_abund[[11]] <- glmmTMB(scaled_met ~ strata + elevation +                               (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[12]] <- glmmTMB(scaled_met ~ poly(strata, 2) + elevation +                      (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[13]] <- glmmTMB(scaled_met ~ poly(strata, 3) + elevation +                      (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
# strata method
...
# strata, elevation, and continent 
mods_abund[[17]] <- glmmTMB(scaled_met ~ strata + elevation + continent +                   (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[18]] <- glmmTMB(scaled_met ~ poly(strata, 2) + elevation + continent +          (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[19]] <- glmmTMB(scaled_met ~ poly(strata, 3) + elevation + continent +          (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
# strata, method, and continent 
...
# strata, method, and elevation 
mods_abund[[23]] <- glmmTMB(scaled_met ~ strata + method + continent + elevation +          (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[24]] <- glmmTMB(scaled_met ~ poly(strata, 2) + method + continent + elevation + (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[25]] <- glmmTMB(scaled_met ~ poly(strata, 3) + method + continent + elevation + (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
# strata interactions with methods or continent or elevation 
...
mods_abund[[28]] <- glmmTMB(scaled_met ~ strata*elevation +                                 (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
# strata interactions with methods or continent or elevation plus additive relationships continent and elevation and methods 
mods_abund[[29]] <- glmmTMB(scaled_met ~ strata*method + continent +                        (strata|link), family = beta_family(link = "logit"), data = abund)
mods_abund[[30]] <- glmmTMB(scaled_met ~ strata*method + elevation +                        (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[31]] <- glmmTMB(scaled_met ~ strata*continent + method +                        (strata|link), family = beta_family(link = "logit"), data = abund)
mods_abund[[32]] <- glmmTMB(scaled_met ~ strata*continent + elevation +                     (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[33]] <- glmmTMB(scaled_met ~ strata*elevation + method +                        (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
mods_abund[[34]] <- glmmTMB(scaled_met ~ strata*elevation + continent +                     (strata|link), family = beta_family(link = "logit"), data = abund) # returns NA/NaN
```
...
bbmle::AICtab(mods_abund, mnames = unlist(abund_mod_names))
```
...
ggplot(abund, aes(y = predictions_fit, x = strata,
  ...
  facet_wrap(~link + taxa) + theme_bw() + 
  scale_fill_viridis_d("Taxa") + theme_bw() +
  theme(legend.position = c(0.54,0.05), legend.box = "horizontal");
```
...
taxa_link_slope$taxa_order
(taxa_link_slope_avg <- taxa_link_slope %>%
    select(taxa_order, estimate, se, CI_95_lower, CI_95_upper, CI_99_lower, CI_99_upper, CI_80_lower, CI_80_upper, sig, metric) %>%
...
col_2_slope_rich_est <- ggplot(taxa_link_slope_rich, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  geom_hline(yintercept = 0, alpha = 0.5, size = 1) + geom_point(size = 1.5) + 
  geom_linerange(aes(ymax = CI_80_upper, 
                     ymin = CI_80_lower), size = 1.1) +
  geom_linerange(aes(ymax = CI_95_upper,
                     ymin = CI_95_lower)) +
  ...
  annotate("text", x = seq(49,47), y = rep(4,3), 
           label = c("  * = 80% CI excludes 0"," ** = 95% CI excludes 0","*** = 99% CI excludes 0"),
           size = 3) +
  scale_color_viridis_d("Taxa") + theme_bw() +
  theme(legend.position = "none", axis.text.x.top = element_text(size = 8),
        axis.ticks.length.x.top = unit(0, "cm")); col_2_slope_rich_est
...
col_2_slope_abund_est <- ggplot(taxa_link_slope_abund, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  geom_text(aes(x = order, y = if_else(sign(estimate) == 1, CI_95_upper + 1, CI_95_lower - 1), label = sig), show.legend = F) +
  scale_color_viridis_d("Taxa") + theme_bw() + 
  theme(legend.position = "right", axis.text.x.top = element_text(size = 8),
        axis.ticks.length.x.top = unit(0, "cm")); col_2_slope_abund_est

col_2_slope_est <- ggarrange(col_2_slope_rich_est, col_2_slope_abund_est, widths = c(1,1.3)); col_2_slope_est

col_2_slope_rich_avg <- ggplot(taxa_link_slope_rich_avg, 
  ...
  ggtitle("                   Species Richness") + coord_flip() + 
  geom_hline(yintercept = 0, alpha = 0.5, size = 1) + geom_point(size = 1.5) + 
  geom_linerange(aes(ymax = CI_80_upper, 
                     ymin = CI_80_lower), size = 1.1) +
  geom_linerange(aes(ymax = CI_95_upper,
  ...
  theme(legend.position = "none", 
        ...
        axis.ticks.length.x.top = unit(0, "cm")); col_2_slope_rich_avg

col_2_slope_abund_avg <- ggplot(taxa_link_slope_abund_avg, 
                                aes(x = factor(taxa_order, levels = c(
                                "Amphibians", "Small mammals", "Birds", "Bats"
                               )), y = estimate, col = taxa_order)) + 
  ylab(expression(Delta ~ "metric with 1% increase in height")) + xlab(" ") + 
  ggtitle("                           Abundance") + coord_flip() + 
  geom_hline(yintercept = 0, alpha = 0.5, size = 1) + geom_point(size = 1.5) + 
  geom_linerange(aes(ymax = CI_80_upper, 
                     ymin = CI_80_lower), size = 1.1) +
  geom_linerange(aes(ymax = CI_95_upper,
  ...
  scale_y_continuous(limits = c(-10,10), breaks = seq(-10, 10, by = 2.5), 
                     sec.axis = sec_axis(~., breaks = c(-5,0,5), labels = c("Decreasing"," ","Increasing"))) +  scale_color_viridis_d(" ") + theme_bw() + 
  theme(legend.position = "right", 
        axis.text.x.top = element_text(size = 8),
        axis.ticks.length.x.top = unit(0, "cm"),
        legend.text=element_text(color="white")) + guides(color=guide_legend(override.aes=list(fill="white", col = "white"))); col_2_slope_abund_avg

col_2_slope_avg <- ggarrange(col_2_slope_rich_avg, col_2_slope_abund_avg, widths = c(1,1.3)); col_2_slope_avg
ggarrange(col_2_slope_est, col_2_slope_avg, nrow = 2, heights = c(6,1))
ggsave("analysis/figures/predictions_bio_parameters_link_taxa_4_panel.jpeg", width = 12, height = 11, units = "in", dpi = 300)
```

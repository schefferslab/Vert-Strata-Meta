...
## Loading data
```{r loading data}
dat <- read_csv(here("data/stripped_data/final/data_joined.csv")) %>%
...
ggsave(here("figures/richness_strata_full_continent.jpeg"), width = 8, height = 6, units = "in", dpi = 300)
...
mods_abund <- lapply(model_formulas_ch, function(x){
  glmmTMB(as.formula(x), family = beta_family(link = "logit"), 
          weights = bin_weight, 
          data = abund)
  })
...
ggsave(here("figures/abundance_strata_full_continent.jpeg"), width = 8, height = 6, units = "in", dpi = 300)
...
col_2_slope_rich_est_cont <- ggplot(taxa_link_slope_rich, aes(x = order, y = estimate, col = taxa_order)) + 
  ...
  scale_y_continuous(limits = c(-4.5, 4.5), breaks = seq(-4, 4, by = 2), expand = c(0.05, 0),
                     sec.axis = sec_axis(~., breaks = c(-2,0,2), labels = c("Decreasing with height     "," ","     Increasing with height"))) +
  geom_text(aes(x = order, y = if_else(sign(estimate) == 1, CI_95_upper + 1, CI_95_lower - 1), label = sig), show.legend = F) +
  annotate("text", x = seq(58, 56), y = rep(3,3), 
  ...
  theme(legend.position = "none", 
        ...
        axis.text.x.top = element_text(size = 12, vjust = 1.5),
...
col_2_slope_rich_avg_cont <- ggplot(taxa_link_slope_rich_avg, 
  ...
  ylab(expression(Delta ~ "metric per unit increase in height")) + xlab(" ") + coord_flip() + 
  geom_hline(yintercept = 0, alpha = 0.5, size = 0.8) + geom_point(size = 1.5, key_glyph = "rect") + 
  geom_linerange(aes(ymax = CI_80_upper, 
                     ymin = CI_80_lower), size = 1.1) +
  geom_linerange(aes(ymax = CI_95_upper,
                     ymin = CI_95_lower)) +
  ...
  theme(legend.position = c(.5,-0.1), legend.direction = "horizontal", panel.border = element_rect(colour = "black", fill=NA, size=1), 
        axis.text.x.top = element_text(size = 12, vjust = 4),
  ...
  guides(colour = guide_legend(title.position = "top",
                               override.aes = list(size = 4))); col_2_slope_rich_avg_cont
...
col_2_slope_est_cont <- ggarrange(col_2_slope_rich_est_cont, col_2_slope_abund_est_cont, labels = c("(a)","(b)"), label.x = 0.237, label.y = 0.927)
col_2_slope_avg_cont <- ggarrange(col_2_slope_rich_avg_cont, col_2_slope_abund_avg_cont,common.legend = T, legend = "bottom",labels = c("(c)","(d)"), label.x = 0.23, label.y = 0.87)
ggarrange(col_2_slope_est_cont, col_2_slope_avg_cont, nrow = 2, heights = c(9,2))
ggsave(here("figures/parm_taxa_4_panel.jpeg"), width = 11, height = 12, units = "in", dpi = 600)
```
...
taxa_slope_comp <- taxa_link_slope %>%
  select(taxa_order, link, estimate, se, metric, CI_95_lower, CI_95_upper, CI_80_lower, CI_80_upper, sig) %>%
  pivot_wider(values_from = c(estimate, se, metric, CI_95_lower, CI_95_upper, CI_80_lower, CI_80_upper, sig), names_from = metric) %>%
  na.omit 

taxa_slope_comp$sig[taxa_slope_comp$taxa_order=="Bats"] <- "insig"
taxa_slope_comp$sig[taxa_slope_comp$taxa_order=="Amphibians"] <- "insig"
taxa_slope_comp$sig[taxa_slope_comp$taxa_order=="Birds"] <- "sig"
taxa_slope_comp$sig[taxa_slope_comp$taxa_order=="Small mammals"] <- "sig"


gg_4_panel_slope_comp <- ggplot(data = taxa_slope_comp, aes(x = estimate_richness, y = estimate_abundance, col = taxa_order)) + 
  stat_smooth(method = "lm", fullrange = T, alpha = 0.5, fill = "grey80", size = 0.5, aes(linetype = sig)) +
  stat_cor(aes(label = ..r.label..), label.x = 0.4, label.y = -1.1, col = "black", size = rel(3)) +
  stat_cor(aes(label = ..p.label..), label.x = 0.4, label.y = -1.6, col = "black", size = rel(3)) +
  geom_hline(yintercept = 0, linetype = "dashed", col = "grey60", size = 0.75) +
  geom_vline(xintercept = 0, linetype = "dashed", col = "grey60", size = 0.75) +
  geom_point(aes(col = taxa_order), size = 1) + 
  geom_abline(slope=1, intercept=0, alpha = 0.5) +
  coord_cartesian() +
  coord_fixed(xlim = c(-2.5,2.5), ylim = c(-2.5,2.5)) +
  ylab(expression(Delta ~ "Abundance per unit increase in height")) + 
  xlab(expression(Delta ~ "Richness per unit increase in height")) +
  facet_wrap(~taxa_order) +
  scale_linetype_manual(values=rev(c("solid", "dashed")))+
  guides(linetype = FALSE) +
  scale_color_viridis_d(" ") + 
  theme_classic() + 
  theme(legend.position = "bottom"); gg_4_panel_slope_comp
```

```{r}
gg_slope_comp <- ggplot(data = taxa_slope_comp, aes(x = estimate_richness, y = estimate_abundance)) + 
  stat_smooth(method = "lm", fullrange=T, alpha = 0.5, fill = "grey80", size = 0.5, col = "black") +
  stat_cor(aes(label = ..r.label..), label.x = 0.7, label.y = -1.1, col = "black", size = rel(3)) +
  stat_cor(aes(label = ..p.label..), label.x = 0.7, label.y = -1.35, col = "black", size = rel(3)) +
  geom_hline(yintercept = 0, linetype = "dashed", col = "grey60", size = 0.75) +
  geom_vline(xintercept = 0, linetype = "dashed", col = "grey60", size = 0.75) +
  geom_point(aes(col = taxa_order), size = 1.5) + 
  geom_abline(slope=1, intercept=0, alpha = 0.5) +
  ylab(expression(Delta ~ "Abundance per unit increase in height")) + 
  xlab(expression(Delta ~ "Richness per unit increase in height")) +
  coord_cartesian() +
  ...
  scale_color_viridis_d(" ") + 
  theme_classic() + 
  theme(legend.position = "bottom"); gg_slope_comp

ggarrange(gg_slope_comp, 
          gg_4_panel_slope_comp, 
          heights = c(1, 1.07),
          ncol = 1,
          common.legend = T, 
          legend = "bottom"); 
ggsave(here("figures/taxa_slope_comp_combined.jpeg"), width = 5, height = 8, dpi = 600)
```



## Log frequency plots (horizontal and vertical)
```{r}
...
ggsave(here("figures/summary_slopes_taxa_biometric_vert.jpeg"), width = 5, height = 8, units = "in", dpi = 600)
...
ggsave(here("figures/summary_slopes_vars.jpeg"), width = 8, height = 4.5, units = "in", dpi = 600)
...
write_csv(mod_cat_sum, here("tables/mod_cat_sum.csv"))
...
emmeans::emmeans(sum_mods_cat_list[[sig_mods[1]]], pairwise~continent)$contrasts
 ...
 # Africa - Americas              0.016 0.755 25   0.021  0.9998
 # Africa - Asia and Oceania      1.095 0.771 25   1.420  0.3460
 # Americas - Asia and Oceania    1.079 0.398 25   2.710  0.0310
...
emmeans::emmeans(sum_mods_cat_list[[sig_mods[2]]],  pairwise~season)$contrasts
 # contrast                estimate    SE df t.ratio p.value
 # Dry - Sampling combined    0.881 0.476 25   1.852  0.1738
 # Dry - Wet                 -0.767 0.521 25  -1.472  0.3213
 # Sampling combined - Wet   -1.648 0.368 25  -4.472  0.0004
...
emmeans::emmeans(sum_mods_cat_list[[sig_mods[3]]],  pairwise~forest_type_iucn)$contrasts
 # contrast                                    estimate    SE df t.ratio p.value
 # dry_forest - moist_lowland_forest             -0.874 0.480 25  -1.820  0.1836
 # dry_forest - moist_montane_forest              0.537 0.453 25   1.186  0.4725
 # moist_lowland_forest - moist_montane_forest    1.411 0.420 25   3.359  0.0068
...
ggsave(here("figures/abundance_richness_correlation.jpeg"), width = 6, height = 4, units = "in", dpi = 600)
...
dat_table <- read_csv(here("data/stripped_data/final/data_joined.csv")) %>%
  dplyr::filter(taxa != "All mammals") %>%
  dplyr::filter(taxa != "Primates") %>%
  dplyr::mutate(weight = spatial_rank + temporal_bredth_rank + temporal_resolution_rank,
                taxa = as.factor(taxa),
...
write.csv(full_table_S1, here("tables/meta_data_table.csv"))

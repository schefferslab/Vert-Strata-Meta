...
```{r}
phil <- st_crop(ne_sf, ymin = 4.5, ymax = 20, xmin = 114, xmax = 130) %>%
  as("Spatial")
...
coordinates(coords_p) <- ~ longitude + latitude
crs(coords_p) <- proj4string

coords_phil <- coords_p[phil]@coords %>%
  as.data.frame()
...
coords_pan <- coords_ca[Philippines]@coords %>%
  as.data.frame()
...
```{r}
{dat_link_pan <- dat_link[row.names(coords_pan),] %>%
  mutate(longitude_j = longitude + runif(1,-1,2),
         latitude_j = latitude + runif(1,-1,2))
dat_link_pan_jittered <- dat_link_pan %>%
  rename(longitude_o = longitude, 
         latitude_o = latitude) %>%
  pivot_longer(c(longitude_o, longitude_j, latitude_o, latitude_j), names_to = c(".value","Category"), names_sep = "_") 
...
{gg_panama <- ggplot() + 
    ...
    new_scale_fill() +
    geom_path(data = dat_link_pan_jittered, aes(x = longitude, y = latitude, group = link)) + 
    geom_point(data = dat_link_ca, aes(x = longitude, y = latitude, fill = taxa_order), 
                                       size = 5, shape = 21, stroke = 0.5, col = "grey20") +
    geom_point(data = dat_link_pan, aes(x = longitude_j, y = latitude_j, fill = factor(taxa_order, levels = c("Bats","Birds","Small mammals","Amphibians"))), 
               size = 5, shape = 21, stroke = 0.5, col = "grey20") +
    scale_fill_manual("Taxa", values = viridis(4, option = "viridis")) +
    ...
    theme_map() + theme(legend.position = "none",
                        legend.direction = "horizontal",
                        legend.key=element_blank());
ggsave("analysis/figures/study descriptions/rmap_royal2_rev_grey_point_inset.jpeg", width = 5, height = 4, units = "in", dpi = 600); print(gg_panama)}
}
